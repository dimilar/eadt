...
(defcustom android-force-arm nil
  "Set t to generate 32-bit ARM instructions instead of Thumb-1. It defaults to nil"
  :group 'android-mode
  :type '(const :tag "force arm or not"))

(defcustom android-cmake-include-file "/zleinter/emax/android/android.cmake"
  "Some macros for cmake. It is included in CMakeLists.txt"
  :group 'android-mode
  :type 'string
  )

(defun android-gen-file (file-name gen-fun &optional vars)
  "Generate a file named `\"file-name\"' with the function `\"gen-fun\"'.
Replace the variables in the template using the variables `\"vars\"'."
  ;;; ?`find-file-literally'
...
"\"<!-- For native project, it is a dummy file and is used as the sentinel for android mode -->\"")
  "Template for the genearl header part of CMakeLists.txt"
  :group 'android-mode
  :type '(repeat string)
  :set '(lambda (sym val)
...
(defcustom android-cmakelist-header-template
  ...
  "Template for the tail part of CMakeLists.txt given a general android project."
  :group 'android-mode
  :type '(repeat string)
  :set '(lambda (sym val)
...
(defcustom android-cmakelist-tail-template
  ...
  "Template for the tail part of CMakeLists.txt for a native Android project."
  :group 'android-mode
  :type '(repeat string)
  :set '(lambda (sym val)
...
(defcustom android-cmakelist-native-tail-template
  ...
  "Template for the tail part of CMakeLists.txt given a native project."
  :group 'android-mode
  :type '(repeat string)
  :set '(lambda (sym val)
...
"(when (string= android-native-project-type \"c\") (insert \"LDFLAGS += -Wl,--whole-archive -lgcc -Wl,--no-whole-archive  \\n\"))")
 ;;-disable-multilib
  "Template for AndroidMakefile.in"
  :group 'android-mode
  :type '(repeat string)
  :set '(lambda (sym val)
